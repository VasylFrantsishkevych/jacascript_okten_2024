...
// - клонувати його повністю, та додати клон в bod
...
for (const {title, monthDuration} of coursesAndDurationArray) {
    ...
    p.innerText = `Duration ${monthDuration} months`;
...
simpsons.forEach(simpson => {
    ...
    divMain.append(img, h3, h4)
    div.append(divMain, p);
...
coursesArray.forEach(course => {
    ...
    divTime.innerText = `Months: ${monthDuration}`

    let divHour = document.createElement('div');
    divHour.innerText = `Hours: ${hourDuration}`
    ...
    divTime.append(divMonth, divHour);
    container.append(divTitle, divTime, ul);
})
